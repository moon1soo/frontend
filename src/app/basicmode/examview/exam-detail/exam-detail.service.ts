...
export class ExamDetailService {
	...
	secCode: string =  'examDetail';
	
	serverData: Model.ExamList[] = [];
	serverClone: Model.ExamList[] = [];
	clientData = {
		select1: []
	}
	clientIdx: number = 1;
	...
	list(): Observable<any> {	
		...
		if(store.examStore) {
			const data = store.examStore.select1.join(',');
			const txt = data.replace(/'/g,'');
			exam = [txt];
		} 
		if(store.examResultStore) {
			const data = store.examResultStore.select1.join(',');
			const txt = data.replace(/'/g,'');
			examResult = [txt];
		} 
		const body = JSON.stringify({ 
			"basicStore" : { "hspTpCd" : hospital , "lclTpCd": "L1"  },
			"examStore": { "select1" : exam },
			"examResultStore": { "select1": examResult}
		});		
		console.log(this._state.code[this.secCode].url);
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setExamDetailStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setExamDetailStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
