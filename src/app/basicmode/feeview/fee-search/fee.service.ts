...
export class FeeService {
	...
	serverData: Model.FeeList[] = [];
	serverClone: Model.FeeList[] = [];
	...
	list(): Observable<any> {
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setFeeStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setFeeStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
					...
					setTimeout(() => {
						this.selectedCodeList('fee', sendData.filterCd).subscribe(resp => {
