...
export class ConcernService {
	...
	secCode: string =  'concernPatient';
	
	serverData: Model.ConcernList[] = [];
	serverClone: Model.ConcernList[] = [];
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				if(storage) {
					...
					this._service.setConcernPatientStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
					this._service.setConcernPatientStore(sendData);
				}
				// // 선택된 코드 리스트 호출을 위해 getStorage에서 getFilterStorage를 분리함
				// setTimeout(() => {
				// 	this.selectedCodeList('ptNo', sendData.filterCd).subscribe(resp => {
