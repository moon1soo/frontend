...
export class DoctorService {
	...
	serverData: Model.DoctorList[] = [];
	serverClone: Model.DoctorList[] = [];
	clientIdx: number = 1;
	...
	list(): Observable<any> {
		...
		if(store.medicalStore) {
			const data = store.medicalStore.select1.join(',');
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setDoctorStore(sendData);
				...
				if(storage) {
					...
					this._service.setDoctorStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
				...
				setTimeout(() => {
					this.selectedCodeList('visit', sendData.filterCd).subscribe(resp => {
