...
@Component({
 	selector: 'doctor-server',
	templateUrl: './doctor-server.component.html'
})

export class DoctorServerComponent implements OnInit {
	@ViewChild(DxDataGridComponent) dataGrid: DxDataGridComponent;
	@Input('activeTab') activeTab: string;

	dataSource: Model.DoctorList[];
	selectedRowsData: Model.DoctorList;
	totalCount: string;
	resultCount: string;
	allMode: string = 'allPages'; 	loading: boolean = true;
	...
	ngOnInit() {		
		...
		this._dashboard.addGrid$.subscribe(res => {
			if(res === true) {
				// this.droppableGrid(this.tableId);
				this.droppableGrid();
			}
		});
	}
	...
	// 테이블 row 선택 이벤트
	addSelectData(data: Model.DoctorList[]) {
	...
	droppableGrid(): void {
		$('.gridSelect').droppable({
			...
			drop: ( event, ui ) => {
				...
				const parent = event.target.parentNode;
				parent.classList.remove('drag-over-action');
				// return this._dashboard.addData(this.secCode, this.clientIdx, this.selectedRowsData);
				let rows;
