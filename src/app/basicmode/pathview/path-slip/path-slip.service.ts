...
@Injectable()
export class PathSlipService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'pathologySlip';
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				const sendData = {
					...
					client: [],
					filter: [],
					filterCd: 'ordSlipCtgCd'
				};
				// this._service.setPathologyStore(sendData);
				// if(storage) {
				...
				setTimeout(() => {
					this.selectedCodeList('pathology', sendData.filterCd).subscribe(resp => {
