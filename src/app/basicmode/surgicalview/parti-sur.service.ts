...
@Injectable()
export class PartiSurService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'partiSur';

	serverData: Model.PartiSur[] = [];
	serverClone: Model.PartiSur[] = [];

	partiSurStorage: string[];
	...
	list(): Observable<any> {		
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				this._service.setPartiSurStore(sendData);
				if(storage) {
					const list = storage.select1.replace(/'/g, '');
					const arr = list.split(',');
					this.partiSurStorage = arr;
				}				
