...
@Injectable()
export class SurDoctorService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'surDoc';

	serverData: Model.SurDoctor[] = [];
	serverClone: Model.SurDoctor[] = [];
	clientData = {
		select1: []
	}
	clientIdx: number = 1;
	...
	list(): Observable<any> {
		...
		if(store.surgicalMedicalStore) {
			const data = store.surgicalMedicalStore.select1.join(',');
		...
		const body = JSON.stringify({ 
			...
			"surgicalMedicalStore" :  { "select1": surgical }
		});
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setSurDocStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setSurDocStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
				...
				setTimeout(() => {
					this.selectedCodeList('surgical', sendData.filterCd).subscribe(resp => {
