...
@Injectable()
export class PartiAnesthService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'partiAnesth';

	serverData: Model.PartiAnesth[] = [];
	serverClone: Model.PartiAnesth[] = [];

	partiAnesthStorage: string[];
	...
	list(): Observable<any> {		
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				this.serverData = response.allList.map(obj => {
					let robj = {};
					robj['name'] = obj.anstKndNm;
					robj['id'] = obj.anstKndCd;
				...
				this._service.setPartiAnesthStore(sendData);
				if(storage) {
					const list = storage.select1.replace(/'/g, '');
					const arr = list.split(',');
					this.partiAnesthStorage = arr;
				}				
