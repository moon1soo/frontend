...
import { SurMedicalService } from './sur-medical.service';
...
@Component({
 	selector: 'sur-medical-server',
	templateUrl: './sur-medical-server.component.html'
})

export class SurMedicalServerComponent implements OnInit {
	@ViewChild(DxDataGridComponent) dataGrid: DxDataGridComponent;

	dataSource: Model.SurMedical[] = [];
	selectedRowsData: Model.SurMedical;
	totalCount: string;
	resultCount: string;
	...
	constructor(
		...
		private _service: SurMedicalService,
	...
	ngOnInit() {		
		this._dashboard.surgicalMedicalStore$.subscribe(res => {
	...
	// 테이블 row 선택 이벤트
	addSelectData(data: Model.SurMedical[]) {
		this._dashboard.addData(this.secCode, this.clientIdx, data);
	}
	// 테이블 더블클릭
	onRowClick(event: any) {
		const rows = event.component.getSelectedRowsData();
