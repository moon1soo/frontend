...
@Injectable()
export class SurMedicalService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'surMed';
	
	serverData: Model.SurMedical[] = [];
	serverClone: Model.SurMedical[] = [];
	...
	list(): Observable<any> {		
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setSurgicalMedicalStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setSurgicalMedicalStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
