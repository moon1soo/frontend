...
export class SurgicalService {
	...
	secCode: string =  'surgical';

	serverData: Model.SurgicalList[] = [];
	serverClone: Model.SurgicalList[] = [];
	...
	list(): Observable<any> {		
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setSurgicalStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setSurgicalStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
