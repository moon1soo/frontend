...
export class DiagnosisService {
	...
	secCode: string = 'diagnosis';
	
	serverData: Model.DiagnosisList[] = [];
	serverClone: Model.DiagnosisList[] = [];	
	...
	list(): Observable<any> {		
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				this.serverClone = this.serverData.slice(0);
				...
				if(storage) {
					...
					this._service.setDiagnosisStore(data);

					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
					this._service.setDiagnosisStore(sendData);
				}

				// 선택된 코드 리스트 호출을 위해 getStorage에서 getFilterStorage를 분리함
				setTimeout(() => {
					this.selectedCodeList('diagnosis', sendData.filterCd).subscribe(resp => {
