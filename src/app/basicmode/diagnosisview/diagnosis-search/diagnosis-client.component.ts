...
@Component({
 	selector: 'diagnosis-client',
	templateUrl: './diagnosis-client.component.html'
})

export class DiagnosisClientComponent implements OnInit {
	@ViewChild('accDiag') acc;
    ...
    ngOnInit() {
        ...
        let receive = this._dashboard.diagnosisStore$.subscribe(res => {
			...
			setTimeout(() => {
				...
				if(this.activeId) {
					...
					console.log('로드시 idx',this.clientIdx);
				}
			}, 10);
		});	  
	...
	onDeleteRow(data: Model.MedicalList): void {
		console.log('행 삭제 정보', this.dataSource, this.clientIdx);
	...
	beforeChange(event: NgbPanelChangeEvent) {	
		...
		console.log('아코디언 메뉴 변경', this.clientIdx);
