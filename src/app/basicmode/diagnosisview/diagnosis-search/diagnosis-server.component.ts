...
@Component({
 	selector: 'diagnosis-server',
	templateUrl: './diagnosis-server.component.html'
})

export class DiagnosisServerComponent implements OnInit {
	@ViewChild(DxDataGridComponent) dataGrid: DxDataGridComponent;
	@Output() select = new EventEmitter();

	dataSource: Model.DiagnosisList[] = [];
	...
	ngOnInit() {
		...
		this._dashboard.diagnosisStore$.subscribe(res => {
			...
			this.clientLength = res['client'].length;
	...
	// 테이블 row 선택 이벤트
	addSelectData(data: Model.DiagnosisList[]) {
