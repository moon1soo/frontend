...
@Injectable()
export class DiagMedicalService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'diagMed';
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				const sendData = {
					...
					filter: [],
					name: [],
					filterCd: 'dgnsMedDeptNm'
				}
				// this._service.setDiagMedicalStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setDiagMedicalStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
