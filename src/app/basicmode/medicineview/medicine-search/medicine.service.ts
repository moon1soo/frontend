...
export class MedicineService {
	...
	secCode: string =  'medicine';
	
	serverData: Model.MedicineList[] = [];
	serverClone: Model.MedicineList[] = [];
	...
	list(): Observable<any> {
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setMedicineStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setMedicineStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
				...
				setTimeout(() => {
					this.selectedCodeList('medicine', sendData.filterCd).subscribe(resp => {
