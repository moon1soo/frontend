...
export class OrderService {
	...
	secCode: string =  'order';
	
	serverData: Model.OrderList[] = [];
	serverClone: Model.OrderList[] = [];
	...
	list(): Observable<any> {
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				// this._service.setOrderStore(sendData);
				// if(storage) {
				...
				if(storage) {
					...
					this._service.setOrderStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
				...
				setTimeout(() => {
					this.selectedCodeList('order', sendData.filterCd).subscribe(resp => {
