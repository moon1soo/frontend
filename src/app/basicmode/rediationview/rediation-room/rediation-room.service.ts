...
@Injectable()
export class RediationRoomService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'rediationRoom';

	serverData: Model.RediationRoomList[] = [];
	serverClone: Model.RediationRoomList[] = [];
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				if(storage) {
					...
					this._service.setRediationTherapyRoomStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
					this._service.setRediationTherapyRoomStore(sendData);
				}
