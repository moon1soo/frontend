...
@Injectable()
export class RediationRegionService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'rediationRegion';

	serverData: Model.RediationRegionList[] = [];
	serverClone: Model.RediationRegionList[] = [];
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, { headers: this.headers })
			.map(res => {
				...
				if(storage) {
					...
					this._service.setRediationTherapyRegionStore(data);
					setTimeout(() => {
						this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
					}, 100);
				} else {
					this._service.setRediationTherapyRegionStore(sendData);
				}
				// 선택된 코드 리스트 호출을 위해 getStorage에서 getFilterStorage를 분리함
				setTimeout(() => {
					this.selectedCodeList('rediation', sendData.filterCd).subscribe(resp => {
