...
import { NursPlanService } from './nurs-plan.service';
...
@Component({
 	selector: 'nurs-plan-server',
	templateUrl: './nurs-plan-server.component.html'
})

export class NursPlanServerComponent implements OnInit {
	@ViewChild(DxDataGridComponent) dataGrid: DxDataGridComponent;

	dataSource: Model.NursPlan[] = [];
	selectedRowsData: Model.NursPlan;
	totalCount: string;
	resultCount: string;
	...
	constructor(
		...
		private _service: NursPlanService,
	...
	ngOnInit() {
		this._dashboard.nursPlanStore$.subscribe(res => {
	...
	// 테이블 row 선택 이벤트
	addSelectData(data: Model.NursPlan[]) {
