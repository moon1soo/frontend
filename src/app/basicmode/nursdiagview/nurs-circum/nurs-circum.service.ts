...
@Injectable()
export class NursCircumService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string = 'nursCircum';
	serverData: Model.NursCircum[] = [];
	serverClone: Model.NursCircum[] = [];
	...
	list(): Observable<any> {		
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`,body, { headers: this.headers })
			.map(res => {
				...
				this._service.setNursCircumStore(sendData);
				if(storage) {
