...
@Component({
	selector: 'exam-interim',
	templateUrl: './exam-interim.component.html'
})

export class ExamInterimComponent implements OnInit {
	@ViewChild(DxDataGridComponent) dataGrid: DxDataGridComponent;

	dataSource: Model.ExamInterim[] = [];
    totalCount: string;
	seqCode: string = 'exam';
	loading: boolean;
	...
	ngOnInit() {
		...
		if (this._store.store.examResultStore) {
			this.listUrl = 'work/result/examResult.json?';
			this.sqlUrl = 'getInterimResultExamResultSqlView.json';
			this.seqCode = 'examResult';
			this.resultExists = true;
		} else {
			this.listUrl = 'work/result/exam.json?';
			this.sqlUrl = 'getInterimResultExamSqlView.json';
			this.seqCode = 'exam';
			this.resultExists = false;
		}

		const store = this._store.store;
		let seq = this.seqCode.toUpperCase();
		this._service.setSql(null);
		this._service.setSeq(this.seqCode);

		// 중간결과 SQL 불러오기
		this._service.getSQLResult(this.sqlUrl).subscribe(res => {
	...
	onLoadData() {
		...
		this._service.getQueryResult(this.listUrl + 'page=' + this.currentPage).subscribe(res => {
			setTimeout(() => { 
				this.loading = false; 
