...
@Injectable()
export class NursStateService {
  appurl: string;
  headers = new Headers({
    "Accept": "text/plain;charset=UTF-8"
  });
  secCode: string =  'nursState';

  serverData: Model.NursState[] = [];
  serverClone: Model.NursState[] = [];
	...
	list(): Observable<any> {
		...
		return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`,body, { headers: this.headers })
			.map(res => {
				...
				this._service.setNursStateStore(sendData);
				if(storage) {
				...
				setTimeout(() => {
					this.selectedCodeList('nursState', sendData.filterCd).subscribe(resp => {
