...
@Component({
 	selector: 'nurs-state-eav-client',
	templateUrl: './nurs-state-eav-client.component.html'
})

export class NursStateEavClientComponent implements OnInit {
	@ViewChild('accNursStateEav') acc;
	@ViewChild('freetext') freetextPop: any;
	@ViewChild('range') rangePop: any;
	@ViewChild('PN') PNPop: any;

    dataSource: any[] = [];
    selectedRowsData: Model.NursStateEav;
    totalCount: string;
	...
	nursStateEavConditionForm: FormGroup = new FormGroup({});
    ...
    ngOnInit() {
		...
		this.nursStateEavConditionForm = this._fb.group({
			'word-index-0': ['']
		});

        let receive = this._dashboard.nursStateEavStore$.subscribe(res => {
	...
	setDynamicFormIndex(data: string[]): void {
		...
		for(let i = 1; i<data.length; i++) {
			...
			this.nursStateEavConditionForm.addControl(`word-index-${i}`, text);
		}
	}
	// 색인 변경 시 저장
	...
	setDynamicForm(res: any) {
		...
		Array.from(res['client']).forEach((obj, index) => {
			...
			this.nursStateEavConditionForm.addControl(`rangeDtSt-${index}`, start);
			this.nursStateEavConditionForm.addControl(`rangeDtEd-${index}`, end);
			this.nursStateEavConditionForm.addControl(`pn1a-${index}`, pnactrl);
			this.nursStateEavConditionForm.addControl(`pn1p-${index}`, pnpctrl);
			this.nursStateEavConditionForm.addControl(`pn1n-${index}`, pnnctrl);

			freetext.forEach((obj, idx) => {
				this.nursStateEavConditionForm.addControl(`freeText-${index}-${obj.title}`, obj.FormControl);
			});
			freecontrol.forEach((obj, idx) => {
				this.nursStateEavConditionForm.addControl(`freeTextCondition-${index}-${obj.title}`, obj.FormControl);
			});
		});
	}
	// freetext 추가
	addFreeText(event: any): void {
		...
		this.nursStateEavConditionForm.addControl(`freeText-${idx}-${target.freeText.length-1}`, txt);
		this.nursStateEavConditionForm.addControl(`freeTextCondition-${idx}-${target.freeText.length-1}`, condition);
	}
	// freetext control 삭제
	delFreeText(event): void {
		...
		setTimeout(() => {
			this.nursStateEavConditionForm.removeControl(`freeText-${idx}-${seq}`);
			this.nursStateEavConditionForm.removeControl(`freeTextCondition-${idx}-${seq}`);
			this.nursStateEavConditionForm.updateValueAndValidity();
	...
	// 폼 변경여부 관찰
	watchForm(): void {
		this.nursStateEavConditionForm.valueChanges
			// .debounceTime(800)
			.subscribe(res => {
				// console.log(res);
				const arr = [];
	...
	deleteClinentAction(idx: number) {
		this.nursStateEavConditionForm.removeControl(`word-index-${idx}`);
		if(this.clientWordIdx.idx.length > 1) {
			...
			this.nursStateEavConditionForm.addControl(`word-index-0`, text);
		}
	}
    onContentReady(e) {
	...
	addGrid(event): void {
		...
		this.nursStateEavConditionForm.addControl(`word-index-${indexof}`, text);
