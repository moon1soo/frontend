...
@Injectable()
export class NursStateEavService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'nursStateEav';
	serverData: Model.NursStateEav[] = [];
	serverClone: Model.NursStateEav[] = [];
	...
	list(): Observable<any> {		
		...
		if(store.nursStateStore) {
			const data = store.nursStateStore.select1.join(',');
		...
		const body = JSON.stringify({ 
			...
			"nursStateStore": { "select1": nursState }
		});
		...
		.map(res => {
			...
			// this._service.setNursStateEavStore(sendData);
			if(storage) {
				...
				this._service.setNursStateEavStore(data);
				setTimeout(() => {
