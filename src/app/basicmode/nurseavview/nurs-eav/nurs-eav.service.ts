...
@Injectable()
export class NursEavService {
	appurl: string;
	headers = new Headers({
		"Accept": "text/plain;charset=UTF-8"
	});
	secCode: string =  'nursEav';

	serverData: Model.NursEav[] = [];
	serverClone: Model.NursEav[] = [];
	...
	list(): Observable<any> {
		...
		.map(res => {
			...
			const sendData = {
				...
				freeText: [],
				freeTextCondition: [],
				filterCd: 'nrVocEntAtrItemNm'
			};
			// this._service.setNursEavStore(sendData);
			// if(storage) {
			// 	this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
			// 	if(storage.wrtDtSt) {
			// 		this.dateStart = this._app.dateConvertNgb(storage.wrtDtSt);
			// 	}
			// 	if(storage.wrtDtEd) {
			// 		this.dateEnd = this._app.dateConvertNgb(storage.wrtDtEd);
			// 	}
			// }
			if(storage) {
				...
				this._service.setNursEavStore(data);
				setTimeout(() => {
					this._service.getStorage(this.secCode, this._state.code[this.secCode].idx, storage);
				}, 100);
			} else {
			...
			setTimeout(() => {
				this.selectedCodeList('nursEav', sendData.filterCd).subscribe(resp => {
	...
	getDeptList(): Observable<any> {
		...
		const url = 'getNursDeptList.json';
