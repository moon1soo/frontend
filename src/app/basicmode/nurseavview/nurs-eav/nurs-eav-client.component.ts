...
@Component({
 	selector: 'nurs-eav-client',
	templateUrl: './nurs-eav-client.component.html'
})

export class NursEavClientComponent implements OnInit {
	@ViewChild('accNursEav') acc;
	...
	nursEavConditionForm: FormGroup = new FormGroup({});
    ...
    ngOnInit() {
		...
		this.nursEavConditionForm = this._fb.group({
			'word-index-0': ['']
		});

        let receive = this._dashboard.nursEavStore$.subscribe(res => {
			...
			Array.from(res['client']).forEach((obj, index) => {
				...
				console.log('[formCtrlSet]', this.formCtrlSet);
			});
			Array.from(res['client']).forEach((obj, index) => {
	...
	setDynamicFormIndex(data: string[]): void {
		...
		for(let i = 1; i<data.length; i++) {
			...
			this.nursEavConditionForm.addControl(`word-index-${i}`, text);
		}
	}
	// 색인 변경 시 저장
	...
	setDynamicForm(res: any) {
		...
		Array.from(res['client']).forEach((obj, index) => {
			...
			this.nursEavConditionForm.addControl(`rangeDtSt-${index}`, start);
			this.nursEavConditionForm.addControl(`rangeDtEd-${index}`, end);
			this.nursEavConditionForm.addControl(`pn1a-${index}`, pnactrl);
			this.nursEavConditionForm.addControl(`pn1p-${index}`, pnpctrl);
			this.nursEavConditionForm.addControl(`pn1n-${index}`, pnnctrl);

			freetext.forEach((obj, idx) => {
				this.nursEavConditionForm.addControl(`freeText-${index}-${obj.title}`, obj.FormControl);
			});
			freecontrol.forEach((obj, idx) => {
				this.nursEavConditionForm.addControl(`freeTextCondition-${index}-${obj.title}`, obj.FormControl);
			});
		});
	}
	// freetext 추가
	addFreeText(event: any): void {
		...
		this.nursEavConditionForm.addControl(`freeText-${idx}-${target.freeText.length-1}`, txt);
		this.nursEavConditionForm.addControl(`freeTextCondition-${idx}-${target.freeText.length-1}`, condition);
	}
	// freetext control 삭제
	delFreeText(event): void {
		...
		setTimeout(() => {
			this.nursEavConditionForm.removeControl(`freeText-${idx}-${seq}`);
			this.nursEavConditionForm.removeControl(`freeTextCondition-${idx}-${seq}`);
			this.nursEavConditionForm.updateValueAndValidity();
	...
	// 폼 변경여부 관찰
	watchForm(): void {
		this.nursEavConditionForm.valueChanges
			// .debounceTime(800)
			.subscribe(res => {
				// console.log(res);
				const arr = [];
	...
	deleteClinentAction(idx: number) {
		this.nursEavConditionForm.removeControl(`word-index-${idx}`);
		if(this.clientWordIdx.idx.length > 1) {
			...
			this.nursEavConditionForm.addControl(`word-index-0`, text);
		}
	}
	// 그리드 추가
	addGrid(event): void {
		...
		this.nursEavConditionForm.addControl(`word-index-${indexof}`, text);
