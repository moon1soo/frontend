...
@Component({
 	selector: 'nurs-eav-items-client',
	templateUrl: './nurs-eav-items-client.component.html'
})

export class NursEavItemsClientComponent implements OnInit {
	@ViewChild('accNursEavItems') acc;
	...
	nursEavItemsConditionForm: FormGroup = new FormGroup({});
    ...
    ngOnInit() {
		...
		this.nursEavItemsConditionForm = this._fb.group({
			'word-index-0': ['']
		});

        let receive = this._dashboard.nursEavItemsStore$.subscribe(res => {
	...
	setDynamicFormIndex(data: string[]): void {
		...
		for(let i = 1; i<data.length; i++) {
			...
			this.nursEavItemsConditionForm.addControl(`word-index-${i}`, text);
		}
	}
	// 색인 변경 시 저장
	...
	setDynamicForm(res: any) {
		...
		Array.from(res['client']).forEach((obj, index) => {
			...
			this.nursEavItemsConditionForm.addControl(`rangeDtSt-${index}`, start);
			this.nursEavItemsConditionForm.addControl(`rangeDtEd-${index}`, end);
			this.nursEavItemsConditionForm.addControl(`pn1-${index}`, pnctrl);

			freetext.forEach((obj, idx) => {
				this.nursEavItemsConditionForm.addControl(`freeText-${index}-${obj.title}`, obj.FormControl);
			});
			freecontrol.forEach((obj, idx) => {
				this.nursEavItemsConditionForm.addControl(`freeTextCondition-${index}-${obj.title}`, obj.FormControl);
			});
		});
	}
	// freetext 추가
	addFreeText(event: any): void {
		...
		this.nursEavItemsConditionForm.addControl(`freeText-${idx}-${target.freeText.length-1}`, txt);
		this.nursEavItemsConditionForm.addControl(`freeTextCondition-${idx}-${target.freeText.length-1}`, condition);
	}
	// freetext control 삭제
	delFreeText(event): void {
		...
		setTimeout(() => {
			this.nursEavItemsConditionForm.removeControl(`freeText-${idx}-${seq}`);
			this.nursEavItemsConditionForm.removeControl(`freeTextCondition-${idx}-${seq}`);
			this.nursEavItemsConditionForm.updateValueAndValidity();
	...
	// 폼 변경여부 관찰
	watchForm(): void {
		this.nursEavItemsConditionForm.valueChanges
			// .debounceTime(800)
			.subscribe(res => {
				// console.log(res);
				const arr = [];
	...
	deleteClinentAction(idx: number) {
		this.nursEavItemsConditionForm.removeControl(`word-index-${idx}`);
		if(this.clientWordIdx.idx.length > 1) {
			...
			this.nursEavItemsConditionForm.addControl(`word-index-0`, text);
		}
	}
    onContentReady(e) {
	...
	addGrid(event): void {
		...
		this.nursEavItemsConditionForm.addControl(`word-index-${indexof}`, text);
