...
@Injectable()
export class NursEavItemsService {
  appurl: string;
  headers = new Headers({
    "Accept": "text/plain;charset=UTF-8"
  });
  secCode: string = 'nursEavItems';
  serverData: Model.NursEavItems[] = [];
  serverClone: Model.NursEavItems[] = [];
  ...
  list(): Observable<any> {
    ...
    if (store.nursEavStore) {
      const data = store.nursEavStore.select1.join(',');
    ...
    return this._http.post(`${this.appurl}${this._state.code[this.secCode].url}`, body, {headers: this.headers})
      .map(res => {
        ...
        this._service.setNursEavItemsStore(sendData);
        if (storage) {
