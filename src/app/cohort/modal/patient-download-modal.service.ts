...
@Injectable()
export class PatientDownloadModalService {
...
//                 {seq: string; mode: string; stfNo: string; tblNm:string, idx: string; startDate: string; endDate: string; anonymous: any; confirm: string; modified: string; url: string;}
    ...
    var params = { tableIdx: data.idx, stfNo: data.stfNo, tblNm: data.tblNm, TBL_ID: data.idx
      , anonymous: data.anonymous, modified:data.modified };

		return this._http.post(`${this.appurl}${data.url}`, JSON.stringify(params), { headers: this.headers })
			.map(res => {
